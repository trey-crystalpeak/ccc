...
from .workspace import Workspace, WORKDIR
...
You are working autonomously. The user is not watching — they will review your work later. If you are truly stuck, you may ask for clarification, but prefer to make reasonable assumptions and keep moving."""
...
The agent's summary is a starting point — trust it, but verify. The diff above gives you a head start, but you can also read files, run tests, or do anything else that helps you assess the situation.
...
def _system_prompt(workspace: Workspace) -> str:
    return SYSTEM_PROMPT.format(workspace_id=workspace.id)
...
def _review_loop(workspace: Workspace, system_prompt: str, base_commit: str) -> None:
    ...
    while True:
        ...
        summary_result = _exec_claude(workspace, workspace.agent_summarizer.command(fork_prompt, fork=True))
        ...
        if status == "not_complete":
            workspace.handle_event(Event.NOT_COMPLETE)
            retry_result = _exec_claude(workspace, workspace.agent_worker.command(classification["comment"], system_prompt=system_prompt))
            _log(workspace, f"Worker finished (${retry_result.total_cost_usd:.4f})")
            continue

        if status == "work_done":
            workspace.container.exec("git add -A && git diff --cached --quiet || git commit -m 'workspace changes'", workdir=WORKDIR)
            workspace.handle_event(Event.WORK_DONE)
            return

        if status == "answer":
            task.completed = True
            workspace.handle_event(Event.ANSWER)
            return

        if status == "needs_input":
            workspace.handle_event(Event.NEEDS_INPUT)
            return
...
def _auto_prompts(workspace: Workspace, system_prompt: str, base_commit: str) -> None:
    run_auto_prompts(workspace, system_prompt, base_commit,
                     exec_claude=_exec_claude, review_loop=_review_loop)
...
def create_task(workspace: Workspace) -> None:
    ...
    try:
        ...
        naming_result = _exec_claude(workspace, workspace.agent_namer.command(NAMING_PROMPT.format(prompt=task.prompt)))
        ...
        worker_result = _exec_claude(workspace, workspace.agent_worker.command(task.prompt, system_prompt=system_prompt))
...
def resume_task(workspace: Workspace, message: str) -> None:
    ...
    try:
        ...
        base_commit = workspace.head_commit()
        system_prompt = _system_prompt(workspace)
        worker_result = _exec_claude(workspace, workspace.agent_worker.command(message, system_prompt=system_prompt))
        _log(workspace, f"Worker finished (${worker_result.total_cost_usd:.4f})")
        _review_loop(workspace, system_prompt, base_commit)
        _auto_prompts(workspace, system_prompt, base_commit)
    except Exception as e:
...
def resolve_conflicts(workspace: Workspace) -> None:
    ...
    try:
        ...
        system_prompt = _system_prompt(workspace)
        worker_result = _exec_claude(workspace, workspace.agent_worker.command(MERGE_CONFLICT_MESSAGE, system_prompt=system_prompt))
        _log(workspace, f"Worker finished (${worker_result.total_cost_usd:.4f})")
        _review_loop(workspace, system_prompt, base_commit)
        _auto_prompts(workspace, system_prompt, base_commit)
    except Exception as e:

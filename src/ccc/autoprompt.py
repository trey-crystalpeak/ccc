...
def _lines_changed(workspace, base_commit: str) -> int:
    ...
    result = workspace.container.exec(f"git diff {base_commit}..HEAD", workdir=WORKDIR)
    count = 0
...
def _evaluate_condition(workspace, condition: dict, base_commit: str, exec_claude) -> bool:
    ...
    if ctype == "lines_changed":
        count = _lines_changed(workspace, base_commit)
        if "gt" in condition:
            return count > condition["gt"]
        return False

    if ctype == "agent_question":
...
def run_auto_prompts(workspace, system_prompt: str, base_commit: str,
                     *, exec_claude, review_loop) -> None:
    prompts = _load_auto_prompts(workspace.state.project_path)
    if not prompts:
        return
    _run(workspace, system_prompt, prompts, base_commit,
         exec_claude=exec_claude, review_loop=review_loop)


def _run(workspace, system_prompt, prompts, base_commit,
         *, exec_claude, review_loop) -> None:
    ...
    for ap in prompts:
        ...
        exec_claude(workspace, workspace.agent_worker.command(ap["prompt"], system_prompt=system_prompt))
        review_loop(workspace, system_prompt, auto_base)

        if "then" in ap:
            _run(workspace, system_prompt, ap["then"], auto_base,
                 exec_claude=exec_claude, review_loop=review_loop)
